...
cd qt5
call configure.bat -opensource -confirm-license -static -nomake examples -openssl-linked -I%OPENSSL%\include OPENSSL_LIBS=" -L%OPENSSL% -lUser32 -lAdvapi32 -lGdi32 -lCrypt32 -lws2_32 -llibssl -llibcrypto" -prefix %CD%\..\..\Qt-5.14.2_%GENARCH%
@if not %ERRORLEVEL% == 0 GOTO buildfail
nmake
@if not %ERRORLEVEL% == 0 GOTO buildfail
nmake install
